<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        // Update the status of daftar_poli (Remove the updateStatus query as we are removing status_periksa column)
        // $updateStatus = "UPDATE daftar_poli SET status_periksa = '1' WHERE id = '$id'";
        // $query = mysqli_query($mysqli, $updateStatus);

        // Insert into the periksa table
        ...
        if ($queryInsertPeriksa) {
            ...
            foreach ($arrayObat as $obat) {
                ...
                if ($queryDetailPeriksa) {
                    echo '<script>alert("Pasien telah diperiksa");window.location.href="../../memeriksaPasien.php"</script>';
                } else {
                    echo '<script>alert("Error");window.location.href="../../memeriksaPasien.php"</script>';
                }
            }
        } else {
            echo '<script>alert("Error inserting periksa");window.location.href="../../memeriksaPasien.php"</script>';
        }
    }
?>
